<section class="rounded-2xl border border-slate-200 bg-white p-4 md:p-5 shadow-sm space-y-3">
  <div class="flex items-center justify-between">
    <div>
      <h2 class="text-sm font-semibold text-slate-800">Cargar Data de Cartera</h2>
      <p class="text-[11px] text-slate-500">XLSX (lento) o CSV (rápido). Recomendado CSV.</p>
    </div>
  ...
  <form method="POST" action="{{ route('cargas.data.upload') }}" enctype="multipart/form-data"
    ...
    <div>
      ...
      <input type="file" name="archivo" accept=".xlsx" required
             class="mt-1 block w-72 rounded-lg border border-slate-300 bg-white px-2 py-1.5 text-[11px] shadow-sm">
    </div>
  ...
  <form method="POST" action="{{ route('cargas.data.import.csv') }}" enctype="multipart/form-data"
    ...
    @csrf
    <div>
      <label class="block text-[11px] font-medium text-slate-700">Archivo CSV</label>
      <input type="file" name="csv" accept=".csv,text/csv,text/plain" required
             class="mt-1 block w-72 rounded-lg border border-slate-300 bg-white px-2 py-1.5 text-[11px] shadow-sm">
    </div>
    <button type="submit" class="inline-flex items-center rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs font-medium text-slate-700 shadow-sm hover:bg-slate-50">
      Importar CSV
    </button>
  </form>
</section>
