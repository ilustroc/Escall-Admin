{{-- resources/views/tablas/partials/gestiones.blade.php --}}

<div class="space-y-6">
    
    {{-- SECCIÓN 1: FILTROS Y KPI MENSUAL --}}
    <div class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        <div class="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            ...
            <form method="GET" action="{{ route('tablas.index') }}" class="flex flex-1 flex-col gap-3 md:flex-row md:items-end">
                {{-- Mantenemos la tab activa al filtrar --}}
                <input type="hidden" name="tab" value="gestiones">
            ...
            <table id="tabla" class="complex-table w-full text-left">
                <thead>
                    <tr>
                        ...
                        {{-- Usamos $days para formatear solo el número del día --}}
                        @foreach($days as $day)
                            <th class="text-right">
        ...
        @foreach($wCarteras as $cartera)
            ...
            <div class="rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden">
                <div class="bg-slate-50 px-4 py-2 flex items-center gap-2 border-b border-slate-200">
                     <h4 class="text-xs font-bold text-slate-700 uppercase tracking-wide">{{ $tag }}</h4>
                </div>
